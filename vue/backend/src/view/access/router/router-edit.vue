				...
				<Form ref="form" :model="form" :rules="rules" :label-width="120">
					...
					<FormItem label="上级路由：" prop="pid">
						<Select :style="controlStyle" v-model="form.pid" placeholder="请选择上级路由">
							...
							<Option v-for="router in routers_bk" :label="router.title" :value="router.id" :key="router.id" v-if="router.type==='page'">
...
<script>
	...
	export default {
		...
		watch: {
			show () {
				this.disabled = false
				this.form = Clone(this.router)
        this.form_bk = Clone(this.router)
			},
			routers () {
			  this.routers_bk = Clone(this.routers)
			}
		},
		methods: {
      ...mapActions([
        'updateRouter'
            ...
            this.updateRouter(this.form).then(res => {
              setTimeout(msg, 1)
              this.$Notice.success({
                title: '编辑成功',
                duration: 1.5,
